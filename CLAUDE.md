...
> Если пользователь написал **«релизь»** — выполни ТОЛЬКО:
> ```bash
> ~/Applications/Claude/Dev/release <agent|server|cloud> "описание"
> ```
> Скрипт сам: определяет версию по тегу → коммитит → мёрджит `work/<component>` в `main` → пушит → создаёт тег → пушит тег.
