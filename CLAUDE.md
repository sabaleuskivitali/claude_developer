...
## Передача задач — ОБЯЗАТЕЛЬНО ЧИТАТЬ

> Если пользователь написал что-то вроде **«передай в agent»**, **«перекинь в server»**, **«перекинь в cloud»**, **«скажи агенту»** и т.п. — запиши задачу в `TASKS.md` в корне репо в формате:
> `- [ ] [компонент] описание задачи`
> Подтверди: "✅ Задача добавлена в TASKS.md для [компонент]-сессии"

## Переход в новую сессию — ОБЯЗАТЕЛЬНО ЧИТАТЬ

> Если пользователь написал любую из фраз: **«новая сессия»**, **«переходим»**, **«заканчиваем»**, **«хватит»**, **«стоп»**, **«выходим»** — немедленно выполни `/handoff` без дополнительных вопросов.

## Релиз — ОБЯЗАТЕЛЬНО ЧИТАТЬ

> Если пользователь написал **«релизь»** (с аргументом или без) — выполни ТОЛЬКО:
> ```bash
> ~/Applications/Claude/Dev/release server "аргумент если есть"
> ```
> Скрипт сам: определяет версию по тегу → коммитит → мёрджит `work/server` в `main` → пушит → создаёт тег → пушит тег.
> **Никаких ручных git команд (commit/merge/tag/push) — только через скрипт.**
