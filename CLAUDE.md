...
> Если пользователь написал что-то вроде **«передай в server»**, **«перекинь в cloud»**, **«скажи агенту»** и т.п. — запиши задачу в `TASKS.md` в корне репо в формате:
...
> ```
> Скрипт сам: определяет версию по тегу → коммитит → мёрджит `work/agent` в `main` → пушит → создаёт тег → пушит тег.
