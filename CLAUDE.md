...
```
[Клиентские машины]  →  [Выделенный сервер LAN]  →  [Облако Anthropic]
  Windows Service          Ubuntu 22.04 LTS            Claude Sonnet API
  C# .NET 8                PostgreSQL 16                Vision батч 20 скриншотов
  SQLite WAL буфер         Docker Compose
  SMB sync раз/час         Python ETL + аналитика
```
...
| SMB sync | File.Copy с retry queue | FileSyncWorker (свой) |
...
### SMB авторизация и структура шары

**Авторизация**: Windows Integrated Auth через LocalSystem. В домене работает автоматически. Без домена — в `install.ps1` выполняем `net use \\server\taskming /user:DOMAIN\user password /persistent:yes` перед стартом сервиса.
...
\\server\taskming\
  {machine_id}\
    {YYYYMMDD}\
      events.db          ← SQLite файл за день
      screenshots\
        {event_id}.webp  ← скриншоты за день
```

Каждый день — новый SQLite файл. FileSyncWorker копирует файл за **вчера** (закрытый день), текущий день — только локально. Это исключает конфликты блокировок при копировании.
...
# 3. Генерация MachineId / UserId → appsettings.json
# (выполняет сам агент при первом старте)

# 4. SMB подключение (если нет домена)
net use \\server\diag /user:DOMAIN\svc_diag $password /persistent:yes

# 5. Native Messaging host — регистрация для Chrome и Edge
...
public enum EventType
{
    ...
    HeartbeatPulse,   // раз в минуту: cpu_ms, events_buffered, sync_lag_sec, drift_ms
    SyncCompleted,    // после SMB-цикла: sent_count, failed_count
    ...
    UpdateAvailable,    // обнаружена новая версия на SMB
...
│       ├── Management/
│       │   ├── CommandPoller.cs        ← polling SMB cmd/ раз в 60с → выполнение команд
│       │   ├── UpdateManager.cs        ← проверка обновлений, скачивание, запуск апдейтера
│       │   └── PerformanceMonitor.cs   ← сбор метрик CPU/RAM/очередь каждые 5 мин
│       ├── Storage/
│       │   └── EventStore.cs           ← единственный SqliteConnection
│       ├── Sync/
│       │   └── FileSyncWorker.cs       ← SMB copy, retry sent=0/1/2
│       └── Models/
│           ├── ActivityEvent.cs
│           └── EventType.cs
│
...
│   ├── etl/
│   │   └── load_events.py              ← SQLite файлы с SMB → PostgreSQL (идемпотентно)
│   ├── pipeline/
...
| SMB шара | `\\server\diag` (не `taskming`) |
...
- `HeartbeatPulse` раз в минуту с: `events_buffered` (count WHERE sent=0), `sync_lag_sec` (NOW - last SyncCompleted), `drift_ms`.

### FileSyncWorker

- Синхронизирует только файлы за **вчера и раньше**. Текущий день — только локально.
- `sent`: 0 = pending, 1 = sent, 2 = failed (retry при следующем цикле).
- При недоступности шары: `sent=2`, пишет `LayerError`, ждёт следующего цикла (60 мин).
- После успешного копирования: `UPDATE events SET sent=1, sent_at=? WHERE sent=0`.
- `SyncCompleted` событие после каждого цикла.
...
- ETL идемпотентен: `INSERT ... ON CONFLICT (event_id) DO NOTHING`.
- ETL запускается cron каждый час — читает новые SQLite файлы с SMB шары.
...
Канал связи — та же SMB шара. Новая инфраструктура не нужна.

```
Сервер записывает:  \\server\diag\{machine_id}\cmd\pending.json
Клиент читает:      \\server\diag\{machine_id}\cmd\pending.json  (polling 60с)
Клиент пишет ответ: \\server\diag\{machine_id}\cmd\ack.json
Сервер читает ответ и удаляет pending.json
```
...
// Путь: {SharePath}\{MachineId}\cmd\pending.json
...
Если `pending.json` старше 10 минут — игнорируем (устаревшая команда), пишем ack `expired`.
...
\\server\diag\
  {machine_id}\
    {YYYYMMDD}\
      ...
      screenshots\
    cmd\
      pending.json       ← сервер пишет команду
      ack.json           ← клиент пишет ответ
  updates\
    latest.json          ← текущая актуальная версия
    v1.0\
      WinDiagSvc.zip     ← пакет обновления
      manifest.json      ← версия, SHA256, changelog
    v1.1\
      ...
```
...
```
Сервер:
  1. Загружает новый WinDiagSvc.zip в \\server\diag\updates\v1.1\
  2. Обновляет \\server\diag\updates\latest.json

Клиент (UpdateManager.cs):
  3. При старте + раз в час: читает latest.json с SMB
  4. Сравнивает с текущей версией (из AssemblyVersion)
  5. Если версия новее → скачивает zip, проверяет SHA256
  6. Распаковывает в staging папку
  7. Создаёт Windows Scheduled Task "WinDiagUpdate" → запускает WinDiagUpdater.ps1
  8. Пишет CommandExecuted событие и завершает сервис

WinDiagUpdater.ps1 (автономный, запущен как LocalSystem):
  9.  Ждёт 5 сек (сервис останавливается)
  10. net stop WinDiagSvc
  11. Copy-Item staging\* → "C:\Program Files\Windows Diagnostics\"
  12. net start WinDiagSvc
  13. Удаляет Scheduled Task и staging папку
  14. Пишет ack.json в \\server\diag\{machine_id}\cmd\
```
...
```json
{
  ...
  "package_path": "updates/v1.1/WinDiagSvc.zip",
  "sha256":       "abc123...",
  "min_version":  "1.0.0",
...
```bash
# Выложить новую версию на SMB и уведомить клиентов
python manage.py update --version 1.1.0 --package ./build/WinDiagSvc.zip
...
Новый `BackgroundService`. Каждые **5 минут** собирает метрики и пишет `PerformanceSnapshot` в SQLite → уходит на сервер при следующем SMB-синке.
...
- HTTP/HTTPS с клиента — нет. Только SMB в LAN.
...
| Сеть | SMB доступ до сервера в LAN |
...
Неделя 1: Инфраструктура агента + сервер (параллельно)
  ...
  □ FileSyncWorker (SMB copy, retry sent=0/1/2, SyncCompleted событие)
  ...
  □ load_events.py (idempotent ETL: SQLite с SMB шары → PostgreSQL)
  □ Heartbeat monitor (нет событий > 15 мин → лог алерт)
  □ SMB шара настроена и доступна с клиентских машин

  РЕЗУЛЬТАТ НЕДЕЛИ 1:
  Сервер принимает данные. Агент собирает базовые события.
  Первый прогон ETL проверяет связку клиент → SMB → PostgreSQL.
...
Неделя 2: Все слои захвата (полный агент)
  ...
  □ BrowserMessageHost.cs — Native Messaging stdin/stdout → EventStore
  □ CommandPoller.cs — polling cmd/pending.json каждые 60с
  ...
  РЕЗУЛЬТАТ НЕДЕЛИ 2:
  Агент установлен на все 10 машин → все слои работают →
  SQLite копируется на сервер → видим полный поток событий,
  ...
  проверяем что SMB sync работает независимо на каждой машине
...
Неделя 3: Отладка по реальным данным 10 машин
  ...
  □ Проверка SMB sync: sent=2 (failed) есть? почему?
  □ Исправление найденных проблем, hotfix деплой через install.ps1
