...
public sealed class HttpUpdateManager : BackgroundService
{
    ...
    private async Task CheckForUpdateAsync(CancellationToken ct)
    {
        try
        {
            ...
            if (manifest is null || string.IsNullOrEmpty(manifest.ExeUrl)) return;
            ...
            await ApplyUpdateAsync(manifest, ct);
        }
        catch (Exception ex)
        {
    ...
    private async Task ApplyUpdateAsync(UpdateManifest manifest, CancellationToken ct)
    {
        using var http     = new HttpClient();
        var exeBytes       = await http.GetByteArrayAsync(manifest.ExeUrl, ct);

        var hash = Convert.ToHexString(SHA256.HashData(exeBytes)).ToLower();
        ...
        var stagedExe = Path.Combine(stagingDir, "Seamlean.Agent.exe");
        await File.WriteAllBytesAsync(stagedExe, exeBytes, ct);
